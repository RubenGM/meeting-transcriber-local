...
Si tienes dos resultados del mismo rango, selecciona ambos y pulsa `Fusionar resultados`. Se abre una comparacion lado a lado: haz clic en el texto de izquierda o derecha para copiarlo a la version final y, si hace falta, corrige despues en el campo editable. Cada bloque incluye `▶` para escuchar solo esa porcion del audio. Las filas identicas se marcan en verde y no piden elegir; las diferencias se resaltan en amarillo/naranja. Al guardar, se crea una carpeta `_merged` y los dos resultados originales quedan ocultos del historial principal sin borrar sus archivos.
